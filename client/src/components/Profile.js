...
export default function Profile() {
  ...
  const formik = useFormik({
    ...
    onSubmit : async values => {
      ...
      toast.promise(updatePromise, {
        ...
        error: <b>Error on updating profile</b>
      })
    }
  })
  ...
  if(serverError) return <h1 className="text-xl text-red-500 font-bold">{serverError.message}</h1>
        ...
        <div className="title flex flex-col items-center">
          ...
          <form className='py-1' onSubmit={formik.handleSubmit}>
              ...
              <div className="text-center py-4">
                <span className='text-gray-500'>come back later? <button className='text-red-500' onClick={userLogout}>Logout</button></span>
              </div>
